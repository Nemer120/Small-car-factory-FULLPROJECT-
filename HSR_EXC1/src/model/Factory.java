 ...
 * @return single_instance */
	public class Factory {
		...
		public double profitPerVehiclesTransportation (VehicleTransportation vehicleTransportation) {
			if(vehicleTransportation == null)
				return 0;
			return vehicleTransportation.getPrice()-vehicleTransportation.getCostOfManufacturing();
		}
		...
		public boolean appointmentOfNewDepartmentManager(Department department) throws ParseException {
			...
			LocalDateTime years=null;
				...
				DepartmentManager NewManager = new DepartmentManager(senior.getID(),senior.getFirstName(),senior.getLastName(),senior.getPhoneNumber(),senior.getGender()
						,senior.getDateOfStartWork(),senior.getSalary()*1.5,senior.getDep(),senior.getYearOfBirth(),date,1500.0,senior.getPassword());
		...
		public int totalProfit() {
			double sum=0;
			for (Deal d: allDeals.values()) {
				sum=sum+d.totalDealPrice();
				for(VehicleTransportation v: d.getAllVehicleTransportation())
					sum=sum-v.getCostOfManufacturing();			
			}
			return (int)sum;
		...
		public List<Employee> allEmployees(int k){
			...
			List<Employee> arrNew = new ArrayList<Employee>();
			for(Employee emp : allEmployees.values()) {
				arr.add(emp);
			}
	 ...
	 * using the Compareable interface in the person Class */
		...
		public ArrayList<DepartmentManager> AllBestsDepManger(){
			ArrayList<DepartmentManager> depManagers = new ArrayList<DepartmentManager>();
			for(Employee emp : allEmployees.values()) {
				if(emp instanceof DepartmentManager) {
					depManagers.add((DepartmentManager) emp);
				}
			Collections.sort(depManagers,new CompareManager());
			}
			return depManagers;
		}
